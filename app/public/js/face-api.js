(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    function drawPreview(videoArg, canvasArg, detection) {
        ...
        var _a = null, x = 0, y = 0, width = 0, height = 0;
        detectionArray.forEach(function (det) {
            _a = det instanceof ObjectDetection ? det.box : det, x = _a.x, y = _a.y, width = _a.width, height = _a.height;
        ...
        var sideLength = 1.6 * Math.max(width, height);
        ...
        x = Math.max(x, 0);
        x = Math.min(x, 640 - sideLength);

        // Face detection's top boundary is the eyes. Subtract some margin to invlude forehead
        y -= 20;

        y = Math.max(y, 0);
        y = Math.min(y, 480 - sideLength);

        if (this.prevWidth && this.prevHeight) {
            var deltaX = Math.abs(this.prevX - x);
            var deltaY = Math.abs(this.prevY - y);
            var deltaW = Math.abs(this.prevWidth - width);
            var deltaH = Math.abs(this.prevHeight - height);

            // Do not redraw bounds if the change is small
            if ((deltaW < 50) && (deltaH < 50) && (deltaX < 40) && (deltaY < 40)) {
                x      = this.prevX;
                y      = this.prevY;
        ...
        ctx.drawImage(videoArg, x, y, width, height, 0, 0, 240, 240);
    }
